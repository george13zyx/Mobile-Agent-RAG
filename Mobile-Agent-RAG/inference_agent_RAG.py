...
from MobileAgentRAG.api import inference_chat
from MobileAgentRAG.text_localization import ocr
from MobileAgentRAG.icon_localization import det
from MobileAgentRAG.controller import get_screenshot, start_recording, end_recording
from MobileAgentRAG.agents import (
...
from MobileAgentRAG.agents import add_response, add_response_two_image
from MobileAgentRAG.agents import ATOMIC_ACTION_SIGNITURES
...
from passage_retrieval_operator_client import upload_action_to_server
...
if BACKBONE_TYPE == "OpenAI":
    ...
    KNOWLEDGE_REFLECTION_MODEL = "claude-3-5-sonnet-latest"
...
def run_single_task(
    ...
    future_tasks=[],
    run_name="test",
    log_root=f"logs/{REASONING_MODEL}/mobile_agent_rag",
    task_id=None,
    tips_path=None,
    shortcuts_path=None,
    ...
    while True:
        ...
        ## 
        info_pool.prev_subtask = info_pool.current_subtask
        ...
        info_pool.current_subtask = parsed_result_planning['current_subtask']
        ...
        steps.append({
            ...
            "current_subtask": parsed_result_planning['current_subtask'],
        ...
        print("Current subtask:", info_pool.current_subtask)
        ...
        ### Stopping by planner ###
        if "Finished" in info_pool.current_subtask.strip():
        ...
        ### Inner Loop for Operator Action Decision and Execution ###
        ...
        while not action_success:
            ...
            prompt_rag_action, rag_images = operator.get_rag(info_pool.instruction, info_pool.current_subtask)

            chat_action = operator.init_chat()
            chat_action = add_response("user", prompt_rag_action, chat_action, image=rag_images)
            ...
            if action_object is None:
                if shortcut_error_message == "human_rejected":
                    print("Human rejected the action, trying again.")
                    continue  # Continue to the next iteration, reselect action.
                else:
                    task_end_time = time.time()
                    steps.append({
                        "step": iter,
                ...
                info_pool.last_human_operator_feedback = ""
                action_success = True

        info_pool.last_action = action_object
        info_pool.last_summary = action_description

        # update Operator-RAG knowledge base
        subtask = info_pool.current_subtask
        subtask = re.sub(r'^\s*\d+\.\s*', '', subtask)
        upload_action_to_server(info_pool.last_action, subtask, screenshot_file)
